...
from app.models.repository import RepoFile, Symbol, DependencyEdge, Repository
    ...
    for f in files:
        ...
        for i, part in enumerate(parts):
            ...
            if is_last:
                ...
                existing = current_level[part]
                # We need a dict for the next level
                child_dict: dict[str, FileTreeNode] = {}
                for child in existing.children:
                    child_dict[child.name] = child
                current_level = child_dict
                # Update parent children from dict
                existing.children = list(child_dict.values())
...
async def get_file_detail(file_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    content = None
    if repo:
        file_full_path = PurePosixPath(repo.local_path) / file.path
        from pathlib import Path

        actual_path = Path(str(file_full_path))
    ...
    if analysis:
        ...
        dep_result = await db.execute(
            ...
            .where(DependencyEdge.analysis_id == analysis.id, DependencyEdge.source_path == file.path)
            .distinct()
        )
        ...
        dependent_result = await db.execute(
            ...
            .where(DependencyEdge.analysis_id == analysis.id, DependencyEdge.target_path == file.path)
            .distinct()
        )
